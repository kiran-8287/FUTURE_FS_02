                    ...
                    <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div className="sm:flex sm:items-start">
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 className="text-lg leading-6 font-medium text-gray-900 dark:text-white">Import Leads from CSV</h3>
                                ...
                                {!file ? (
                                    <div
                                        className="mt-4 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-md cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50"
                                    ...
                                    <div className="mt-4 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg flex items-center justify-between">
                                        ...
                                        <button onClick={() => { setFile(null); setPreviewData([]); }} className="text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400">
                                            <X size={20} />
                                        </button>
                                    </div>
                                )}
                                ...
                                {error && (
                                    <div className="mt-4 p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-md text-sm flex items-center">
                                ...
                                {previewData.length > 0 && (
                                    <div className="mt-4">
                                        <h4 className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">Preview (First 5 Rows)</h4>
                                        <div className="overflow-x-auto border border-gray-200 dark:border-gray-700 rounded text-xs">
                                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                                <thead className="bg-gray-50 dark:bg-gray-900/50">
                                                    <tr>
                                                        {headers.map((h, i) => <th key={i} className="px-3 py-2 text-left text-gray-500 dark:text-gray-400 font-medium">{h}</th>)}
                                                    </tr>
                                                </thead>
                                                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                                    {previewData.map((row, i) => (
                                                        <tr key={i}>
                                                            {headers.map((h, j) => <td key={j} className="px-3 py-2 whitespace-nowrap text-gray-700 dark:text-gray-300">{row[h]}</td>)}
                                                        </tr>
                                                    ))}
                                                </tbody>
                                            </table>
